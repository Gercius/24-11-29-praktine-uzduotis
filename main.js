const products = [
    { id: 1, name: "Laptop", category: "Electronics", price: 1200 },
    { id: 2, name: "Smartphone", category: "Electronics", price: 800 },
    { id: 3, name: "Shirt", category: "Clothing", price: 40 },
    { id: 4, name: "Jeans", category: "Clothing", price: 60 },
    { id: 5, name: "Coffee Maker", category: "Appliances", price: 150 },
    { id: 6, name: "Blender", category: "Appliances", price: 120 },
    { id: 7, name: "TV", category: "Electronics", price: 600 },
    { id: 8, name: "Socks", category: "Clothing", price: 10 },
    { id: 9, name: "Microwave", category: "Appliances", price: 180 },
    { id: 10, name: "Smart Watch", category: "Electronics", price: 250 },
];

console.log(filterProductsByCategory("Electronics"));
console.log(filterProductsByCategory("Clothing"));
console.log(filterProductsByCategory("Clothingggg"));
console.log(filterProductsByCategory());

console.log(filterProductsByPrice("asc"));
console.log(filterProductsByPrice("desc"));
console.log(filterProductsByPrice("descasdsda"));
console.log(filterProductsByPrice(1));

function filterProductsByCategory(category) {
    const uniqueCategories = new Set();

    products.forEach((product) => {
        uniqueCategories.add(product.category);
    });

    if (!uniqueCategories.has(category)) {
        console.log("Such category doesn't exist");
        return;
    }

    return products.filter((product) => product.category === category);
}

function filterProductsByPrice(order) {
    switch (order) {
        case "asc":
            return products.toSorted((a, b) => a.price - b.price);
        case "desc":
            return products.toSorted((a, b) => b.price - a.price);
        default:
            console.log("Enter correct sorting order");
            return;
    }
}
